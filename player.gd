...
func get_input(): # Mostly for player movement input rn
...
func _on_npc_name(name):
	print("NPC's name is ", name)
	npc = get_node(name)
...
func _on_area_entered(npp) -> void:
	onArea = true
	npc = get_node(NodePath("/root/Main/" + npp))
	print(npp)
